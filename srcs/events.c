...
/*   Updated: 2023/01/12 14:22:12 by lnaidu           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_actionx1(t_data *program, int x, int i, int j)
{
	...
	if ((program->map.map[i][j + 1]) == 'C')
		(program->map.map[i][j + 1]) = '0';
	if (((program->map.map[i][j + 1]) == 'E') && (program->map.coin == 0))
...
void	ft_actiony1(t_data *program, int y, int i, int j)
{
	...
	program->map.count++;
	ft_printf("number keys pressed -> %d\n", program->map.count);
	if ((program->map.map[i + 1][j]) == 'C')
		(program->map.map[i + 1][j]) = '0';
	if (((program->map.map[i + 1][j]) == 'E') && (program->map.coin == 0))
		exit(mlx_destroy_window(program->mlx, program->window.reference));
}
